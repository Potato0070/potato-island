...
export default function LotteryScreen() {
  ...
  const [potatoIds, setPotatoIds] = useState<string[]>([]); // 🌟 存储真实卡片ID
  ...
  const executeDraw = async () => {
    ...
    try {
      ...
      let isElder = false;
      let mintNftName = ''; 
      let potatoRewardCount = 0; // 记录如果抽到Potato卡要发几张
      ...
      <ScrollView contentContainerStyle={{padding: 20}}>
         <View style={styles.banner}>
            ...
            <View style={styles.balanceBox}>
               <Text style={{color: '#FFF', fontSize: 12}}>金库闲置: Potato卡 x{potatoIds.length}</Text>
            </View>
         </View>
         ...
         <View style={styles.card}>
            ...
            <TouchableOpacity style={[styles.joinBtn, potatoIds.length < 5 && {backgroundColor: '#CCC'}]} onPress={handleDrawClick} disabled={processing || potatoIds.length < 5}>
               <Text style={styles.joinBtnText}>{potatoIds.length < 5 ? '金库现货不足' : '立即投入并开启'}</Text>
            </TouchableOpacity>
         </View>
      </ScrollView>
